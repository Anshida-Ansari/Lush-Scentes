    ...
    try {
      if (req.user) {
        if (req.user.isBlocked) {
          return res.redirect(
            '/login?error=Your account has been blocked by the admin'
          );
        }
        req.session.user = {
          _id: req.user._id,
...
router.get(
  '/forgotPass-otp',
  userLogin,
  profileController.getForgotPassOtpPage
);
...
router.post(
  '/update-password',
  userAuth,
  profileController.changeresetPassword
);
...
router.get(
  ...
  isUserBlocked,
  userAuth,
  checkoutController.loadCheckoutPage
);
router.post(
  '/checkout',
  isUserBlocked,
...
router.get(
  '/checkout-address',
  userAuth,
  checkoutController.loadCheckoutAddress
);
router.post('/add-checkout-address', userAuth, checkoutController.addressPost);
router.get(
  '/edit-address/:id',
  userAuth,
  checkoutController.loadcheckoutEditAddress
);
...
router.get(
  '/transaction-failure',
  checkoutController.loadTransactionFailurePage
);
...
router.post(
  ...
  checkoutController.handlePaymentDismissal
);
