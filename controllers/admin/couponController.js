...
const createCoupon = async (req, res) => {
  try {
    ...
    if (
      isNaN(parsedMinimumPrice) ||
      parsedMinimumPrice <= 0 ||
      parsedMinimumPrice > 100000
    ) {
      return res.status(201).json({
        status: false,
        message:
          'Invalid minimum amount. Please enter a valid number greater than 0 and less than or equal to 100,000',
      });
    }
  ...
  try {
    ...
    if (
      ...
      return res.status(201).json({
        status: false,
        message:
          'Invalid discount amount. Please enter a valid number greater than 0 and less than or equal to 10,000.',
      });
    }
    const parsedMinimumPrice = parseFloat(minimumPrice);
    if (
      ...
      return res.status(201).json({
        status: false,
        message:
          ' Invalid minimum amount. Please enter a valid number greater than 0 and not exceeding 100,000.',
      });
    }
    if (parsedOfferPrice >= parsedMinimumPrice) {
      return res.status(201).json({
        status: false,
        message:
          ' Invalid discount amount. The offer price must be less than the minimum price.',
      });
    }
    const expirationDate = new Date(expireOn);
    if (isNaN(expirationDate) || expirationDate <= new Date()) {
    ...
    if (!couponUpdate) {
      return res.status(201).json({
        status: false,
        message:
          'Coupon not found. Please check the coupon details and try again.',
      });
    }
