...
const calculateCartTotals = (items) => {
  const subtotal = items.reduce((sum, item) => {
    ...
    const itemPrice =
      parseFloat(item.variant.salesPrice || item.variant.regularPrice) || 0;
...
const addToCart = async (req, res) => {
  try {
    ...
    const priceToUse =
      selectedVariant.salesPrice || selectedVariant.regularPrice;
    ...
    if (existingItemIndex > -1) {
      newQuantity =
        cart.items[existingItemIndex].variant.quantity + variant.quantity;
    }
...
const updateCart = async (req, res) => {
  try {
    ...
    const variant = product.variants.find(
      (v) => v.size === cart.items[itemIndex].variant.size
    );
    if (!variant) {
...
const removeFromCart = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: 'Product removed from cart', cart, totals });
  } catch (error) {
