...
const signUp = async (req, res) => {
  try {
    ...
    if (findUser) {
      return res.render('signup', {
        message: 'User with this email aldready exit',
      });
    }
...
const resendOtp = async (req, res) => {
  try {
    ...
    if (!email) {
      return res
        .status(400)
        .json({ sucess: false, message: 'Email not found in session' });
    }
    const otp = generateOtp();
...
const loadShoppingPage = async (req, res) => {
  try {
    ...
    const priceMin = parseInt(req.query.priceMin) || 500;
    const priceMax = parseInt(req.query.priceMax) || 15000;

    if (req.query.priceMin || req.query.priceMax) {
      filters['variants.salesPrice'] = { $gte: priceMin, $lte: priceMax };
      console.log(
        'Applied price filter on variants:',
        filters['variants.salesPrice']
      );
    } else {
    ...
    if (sortBy) {
      switch (sortBy) {
        case 'priceLowToHigh':
          sortOptions = { 'variants.salesPrice': 1 };
          break;
        case 'nameAZ':
    ...
    console.log(
      'All products (for debug):',
      allProducts.map((p) => ({ name: p.productName, variants: p.variants }))
    );
